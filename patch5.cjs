...
const perfectScriptInner = `const { createClient } = supabase;
        ...
        if (navActions) {
          ...
          navActions.innerHTML = \`
            <div class="user-profile-menu-container" style="position: relative;">
              ...
              <div id="accountDropdown" style="display: none; opacity: 0; transition: opacity 150ms ease; position: absolute; right: 0; top: 100%; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.15); min-width: 220px; z-index: 10000; overflow: hidden; padding: 12px 0; text-align: left; margin-top: 8px;">
                <div style="padding: 0 16px;">
                  <span class="dropdown-label" style="font-size: 11px; font-weight: 500; color: #70757a; text-transform: uppercase;">SIGNED IN AS</span>
                  <div style="display: flex; align-items: center; margin-top: 2px;">
                     <span id="dropdownEmail" style="font-size: 13px; font-weight: 600; color: #1a1a1a; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;" title="\${email}">\${email}</span>
                     \${isPlus ? \`<span class="plus-badge" style="margin-left: 6px; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; background: #4285F4; border-radius: 50%; color: white; font-size: 10px; font-weight: bold;" title="GrabRaw+ Member">✦</span>\` : ""}
                  </div>
                </div>
                ...
                <a href="#" id="signOutBtn" class="dropdown-item" style="color: red !important; font-weight: 600; padding: 8px 16px; display: block; text-decoration: none;">Sign Out</a>
              </div>
            </div>
          \`;
      ...
      document.addEventListener("click", function (e) {
        ...
        if (dropdown && dropdown.style.display !== "none") {
          if (!dropdown.contains(e.target) && (!accountBtn || !accountBtn.contains(e.target))) {
            dropdown.style.opacity = '0';
            setTimeout(() => dropdown.style.display = 'none', 150);
          }
        }
...
const files = ["info.html", "location.html", "my-partners.html", "profile.html", "plus.html", "index.html", "register-company.html"];
