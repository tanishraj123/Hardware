...
const startStr = '<!-- BOM Worker Integration Script -->';
const startIdx = html.indexOf(startStr);

if (startIdx === -1) {
  console.log('Could not find BOM Worker Integration Script');
  process.exit(1);
}

// Find the </script> after the begin
...
// The block starts exactly at startStr and ends exactly after the FIRST </script> after startIdx.
const before = html.substring(0, startIdx);
const after = html.substring(endIdx + '</script>'.length);
