...
for (const file of files) {
  ...
  content = content.replace(/function updateNavbar\(user\)\s*\{[\s\S]*?function showLoggedOutNavbar\(\)\s*\{[\s\S]*?\}/, '');
  
  // Clean up any stray `showLoggedOutNavbar()` calls
  content = content.replace(/showLoggedOutNavbar\(\);/g, '');
  // Clean up any `updateNavbar(session.user);` calls
  content = content.replace(/updateNavbar\(session\.user\);/g, '');
